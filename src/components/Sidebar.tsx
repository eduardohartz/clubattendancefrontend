        ...
        <>
            <div className="hidden h-[100vh] w-[225px] bg-greyscale-100 border-r-2 border-r-greyscale-200 lg:flex flex-1 items-center flex-col">
                ...
                <div className="flex flex-1 items-center flex-col gap-2 mt-[25px] mr-3">
                    <Link to={"/dashboard"}><button className={`${buttonClass} ${page == "/dashboard" ? selected : ""}`}><FontAwesomeIcon icon={faHouse} size="lg" className="mr-1" /> Home</button></Link>
                    <Link to={"/dashboard/meetings"}><button className={`${buttonClass} ${page.includes("meeting") ? selected : ""}`}><FontAwesomeIcon icon={faComments} size="lg" className="mr-1" /> Meetings</button></Link>
                    <Link to={"/dashboard/members"}><button className={`${buttonClass} ${page.includes("member") ? selected : ""}`}><FontAwesomeIcon icon={faUsers} size="lg" className="mr-1" /> Members</button></Link>
                    <Link to={"/dashboard/settings"}><button className={`${buttonClass} ${page == "/dashboard/settings" ? selected : ""}`}><FontAwesomeIcon icon={faCog} size="lg" className="mr-1" /> Club Settings</button></Link>
                    {user.admin ? <Link to={"/dashboard/admin"}><button className={`${buttonClass} ${page == "/dashboard/admin" ? selected : ""}`}><FontAwesomeIcon icon={faUserShield} size="lg" className="mr-1" /> Admin</button></Link> : ""}
                    <span className="w-[180px] border-b-2 border-b-greyscale-200 mb-[40px]"></span>
                    <Link to={"/settings"}><button className={`${buttonClass} ${page == "/settings" ? selected : ""}`}><FontAwesomeIcon icon={faUser} size="lg" className="mr-1" /> User Settings</button></Link>
                    <Link to={"/logout"}><button className={`${buttonClass} text-warningred`}><FontAwesomeIcon icon={faRightFromBracket} size="lg" className="mr-1" /> Logout</button></Link>
                </div>
            </div>
